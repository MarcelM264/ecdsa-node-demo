...
function Transfer({ address, setBalance, privateKey }) {
  ...
  async function hashAndSign() {
    ...
    */
      const signatureArray = await secp.sign(hexMessage, privateKey, {
        recovered: true,
      });
  ...
  return (
    <form className="container transfer" onSubmit={transfer}>
      ...
      <button
        ...
        onClick={hashAndSign}
      >Sign the transaction</button>
      <br></br>
      
      <div>Your transaction hash: {hexMessage}</div>
      <br></br>
      <div>Your signature: xxx{signature.slice(-4)}</div>
      <br></br>
      <div>Your recoveryBit: {recoveryBit}</div>

      <button type="submit" className="button" value="Transfer">Transfer</button>
    </form>
  );
}
